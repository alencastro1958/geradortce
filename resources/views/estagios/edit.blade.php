    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-bold text-3xl bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-600 leading-tight">
                {{ __('Editar TCE: ') }} <span class="text-gray-700 font-medium">{{ $estagio->estagiario->nome }}</span>
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white/90 backdrop-blur-xl shadow-2xl sm:rounded-3xl border border-gray-100 overflow-hidden">
                <div class="p-10">
                    <form method="POST" action="{{ route('estagios.update', $estagio->id) }}" class="space-y-8">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="estagiario_id" id="estagiario_id" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow">
                                    @foreach($estagiarios as $estagiario)
                                        <option value="{{ $estagiario->id }}" {{ old('estagiario_id', $estagio->estagiario_id) == $estagiario->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="empresa_concedente_id" id="empresa_concedente_id" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow">
                                    @foreach($empresas as $empresa)
                                        <option value="{{ $empresa->id }}" {{ old('empresa_concedente_id', $estagio->empresa_concedente_id) == $empresa->id ? 'selected' : '' }}>
                            ...
                            <div x-data="supervisorSelector({{ json_encode($supervisoresPorEmpresa) }}, {{ old('supervisor_estagio_id', $estagio->supervisor_estagio_id ?? 'null') }})">
                            ...
                            <!-- Seguradora -->
                            <div>
                                <label for="seguradora_id" class="block text-sm font-bold text-gray-700 mb-2">Seguradora</label>
                                <select name="seguradora_id" id="seguradora_id" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow">
                                    ...
                                    @foreach($seguradoras as $seguradora)
                                        <option value="{{ $seguradora->id }}" {{ old('seguradora_id', $estagio->seguradora_id) == $seguradora->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow">
                                    <option value="pendente" {{ old('status', $estagio->status) == 'pendente' ? 'selected' : '' }}>Pendente</option>
                                    <option value="ativo" {{ old('status', $estagio->status) == 'ativo' ? 'selected' : '' }}>Ativo</option>
                                    <option value="concluido" {{ old('status', $estagio->status) == 'concluido' ? 'selected' : '' }}>Concluído</option>
                                    <option value="rescindido" {{ old('status', $estagio->status) == 'rescindido' ? 'selected' : '' }}>Rescindido</option>
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 border-t border-gray-100 pt-8">
                            ...
                            <div>
                                ...
                                <input type="date" name="data_inicio" id="data_inicio" value="{{ old('data_inicio', $estagio->data_inicio) }}" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow">
                            ...
                            <div>
                                ...
                                <input type="date" name="data_fim" id="data_fim" value="{{ old('data_fim', $estagio->data_fim) }}" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="number" name="carga_horaria_semanal" id="carga_horaria_semanal" value="{{ old('carga_horaria_semanal', $estagio->carga_horaria_semanal) }}" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-gray-100 pt-8">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" step="0.01" name="valor_bolsa" id="valor_bolsa" value="{{ old('valor_bolsa', $estagio->valor_bolsa) }}" class="w-full rounded-xl border-gray-300 pl-10 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow">
                                </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" step="0.01" name="valor_auxilio_transporte" id="valor_auxilio_transporte" value="{{ old('valor_auxilio_transporte', $estagio->valor_auxilio_transporte) }}" class="w-full rounded-xl border-gray-300 pl-10 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow">
                                </div>
                        ...
                        <div class="border-t border-gray-100 pt-8">
                            ...
                            <textarea name="atividades" id="atividades" rows="4" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-shadow">{{ old('atividades', $estagio->atividades) }}</textarea>
...
<script>
function supervisorSelector(allSupervisores, currentSupervisorId) {
    return {
        allSupervisores: allSupervisores,
        supervisores: [],
        selectedEmpresa: null,
        selectedSupervisor: currentSupervisorId ? String(currentSupervisorId) : '',
        init() {
