                    ...
                    <div class="overflow-x-auto rounded-2xl border border-gray-100 shadow-sm">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50/50">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Razão Social / CNPJ</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Contato</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Cidade/UF</th>
                                ...
                                @forelse($empresas as $empresa)
                                    <tr class="hover:bg-blue-50/30 transition-colors duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                            ...
                                            <div class="text-xs">{{ $empresa->telefone }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex justify-end gap-3">
                                                <a href="{{ route('empresas.edit', $empresa->id) }}" class="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" title="Editar">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                    </svg>
                                                </a>
                                                <form action="{{ route('empresas.destroy', $empresa->id) }}" method="POST" class="inline" onsubmit="return confirm('Deseja realmente remover esta empresa?')">
                                                    @csrf
                                                    @method('DELETE')
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                            <div class="flex flex-col items-center">
                                                ...
                                                <p class="text-xl font-medium text-gray-400">Nenhuma empresa encontrada</p>
                                                <a href="{{ route('empresas.create') }}" class="mt-4 text-indigo-600 hover:text-indigo-800 font-semibold">Cadastre a primeira agora &rarr;</a>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
