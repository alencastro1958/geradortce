<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="font-bold text-2xl text-gray-800">Incluir Supervisor de Estágio</h2>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-xl sm:rounded-2xl p-8">
                ...
                @endif
                <form method="POST" action="{{ route('supervisores.store', $empresa) }}" class="space-y-6">
                    @csrf
                    @include('supervisores._form', ['supervisor' => null])
                    <div class="flex justify-end gap-4 pt-6 border-t">
                        <a href="{{ route('empresas.edit', $empresa) }}" class="px-6 py-3 rounded-xl text-gray-700 hover:bg-gray-100">Cancelar</a>
                        <button type="submit" class="px-8 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 shadow-lg">Salvar Supervisor</button>
                    </div>
                </form>
            </div>
        </div>
