<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="font-bold text-2xl text-gray-800">Incluir Representante Legal</h2>
                @php
                    $nomeEntidade = match($tipo) {
                        'empresa' => $entidade->nome_fantasia ?? $entidade->razao_social,
                        'instituicao' => $entidade->nome_fantasia ?? $entidade->razao_social,
                        'seguradora' => $entidade->nome,
                        'agente' => $entidade->nome_fantasia ?? $entidade->razao_social,
                        default => '',
                    };
                @endphp
                <p class="text-sm text-gray-500">{{ $nomeEntidade }}</p>
            </div>
        </div>
    </x-slot>
    <div class="py-12">
            ...
            <div class="bg-white shadow-xl sm:rounded-2xl p-8">
                ...
                <form method="POST" action="{{ route('representantes.store', [$tipo, $entidade->id]) }}" class="space-y-6">
                    ...
                    <div class="flex justify-end gap-4 pt-6 border-t">
                        ...
                        <button type="submit" class="px-8 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-500 hover:to-teal-500 shadow-lg">Salvar Representante</button>
                    </div>
                </form>
            </div>
        </div>
